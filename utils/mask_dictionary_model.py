...
class MaskDictionaryModel:
    ...
    def add_new_frame_annotation_with_id(self, mask_list, box_list, label_list, id_list):
        mask_img = torch.zeros(mask_list.shape[-2:])
        anno_2d = {}
        for idx, (mask, box, label, pid) in enumerate(zip(mask_list, box_list, label_list, id_list)):
            final_index = int(pid)+1
    ...
    def update_masks(self, tracking_annotation_dict, iou_threshold=0.8, objects_count=0):
        updated_masks = {} # detect if exist
        #updated_masks = copy.deepcopy(tracking_annotation_dict.labels) # tracking to the end
        ...
        for seg_obj_id, seg_mask in self.labels.items():  # tracking_masks
                ...
                if iou > iou_threshold:
                    if object_info.class_name==seg_mask.class_name:
                        if (not have_same_class) or iou>max_iou:
                            max_iou_id = object_info.instance_id
                            max_iou = iou
            ...
            #print(seg_obj_id, seg_mask.class_name, max_iou_id)
            flag = max_iou_id
