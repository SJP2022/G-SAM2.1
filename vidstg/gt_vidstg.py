...
annotations_path = "/share_io02_hdd/shijiapeng/STVG_datasets/vidstg/sent_annos/train_annotations.json"
output_path = f"/share_io02_hdd/shijiapeng/STVG_datasets/vidstg/visualization/{vid}_vidstg"
...
for anno in annotations:
    if anno["vid"]==vid:
        used_begin_fid = anno["used_segment"]["begin_fid"]
        used_end_fid = anno["used_segment"]["end_fid"]
        gt_begin_fid = anno["temporal_gt"]["begin_fid"]
        gt_end_fid = anno["temporal_gt"]["end_fid"]
        target_id = anno["captions"][0]["target_id"]
        print(vid, gt_begin_fid, gt_end_fid, target_id)

anno_dict = {}
with open(gt_path, "r", encoding='utf-8') as f:
    gt_data = json.load(f)
for fid, gt_item in enumerate(gt_data["trajectories"]):
    if fid<gt_begin_fid or fid>gt_end_fid:
        continue
    frame_id = str(fid)
    #print(frame_id, gt_item)
    for gt_obj in gt_item:
        ...
        if frame_id not in anno_dict:
            anno_dict[frame_id] = []
        #print(frame_id, object_id, x1, y1, x2, y2)
